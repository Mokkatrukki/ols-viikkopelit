 ...
 */
function extractLocation(cellValue) {
    if (!cellValue)
        return null;
 ...
 */
function extractGameDuration(cellValue) {
    if (!cellValue)
        return null;
 ...
 */
function extractYearFromTeam(teamName) {
    if (!teamName)
        return '';
 ...
 */
function getGameType(sheetName) {
    if (sheetName.includes('5v5'))
        return '5 v 5';
    if (sheetName.includes('4v4'))
        return '4 v 4';
    return '';
}
/**
...
function isTeamName(value) {
    ...
    if (str.length === 0)
        return false;
    if (str.toLowerCase().includes('aloittavat joukkueet'))
        return false;
    if (str.toLowerCase().includes('tötsät löytyy'))
        return false;
    if (str.toLowerCase().includes('viimeiset joukkueet'))
        return false;
    if (str.toLowerCase().includes('kentät'))
        return false;
    if (!/[a-zäöå]/i.test(str))
        return false;
    return true;
}
/**
...
function parseExcelFile(filePath) {
    ...
    const allGames = [];
    const sheetsToProcess = workbook.SheetNames.filter(name => name.includes('5v5 turnaukset') || name.includes('4v4 turnaukset'));

...
function parsePdfUrlDate(url: string): Date | null {
  // Matches both "Viikkopelit-7_5_2026.pdf" and "Viikkopelit_8_5_2025.pdf"
  const m = url.match(/Viikkopelit[-_](\d+)_(\d+)_(\d{4})\.pdf/i);
  if (!m) return null;
...
async function scanDirectory(year: number, month: number): Promise<string[]> {
  ...
  try {
    ...
    return extractPdfUrls(html, url);
  } catch {
    return [];
  }
}
...
export async function fetchLatestPdf(): Promise<PdfInfo | null> {
  ...
  // Scan current and next month's upload directories
  const [curUrls, nextUrls] = await Promise.all([
    scanDirectory(y, mo),
    scanDirectory(nextY, nextMo),
  ]);
  const allUrls = [...new Set([...curUrls, ...nextUrls])];

...
// ----- Interfaces for our extracted data (moved from dataExtractor.ts) -----
...
// New interface for field block information
export interface FieldBlockInfo {
  name: string;
  gameDuration: string;
  gameType: string;
  year: string;
 ...
 */
export function inferYearFromTeams(team1: string, team2: string): string | null {
...
// Placeholder for the debugLog function type, assuming it's passed or defined globally
// For now, we'll assume it's passed as an argument where needed.
export type DebugLogUtil = (message: string, data?: any) => void;

// Further helper functions (detectFieldNamesOnLine, initializeFieldBlocks, etc.) will be added here.

export function detectFieldNamesOnLine(
  line: ExtractedTextElement[],
  pageWidth: number, // Used for isLandscape calculation, though not directly in this snippet
  fieldDetectionTolerance: number,
  debugLog: DebugLogUtil
): { leftFieldEl?: ExtractedTextElement, rightFieldEl?: ExtractedTextElement } {
      ...
      if (fieldNumber.endsWith('A')) {
        ...
        if (existingBField) {
          ...
          const potentialBFields = line.filter(el => {
            if (el === fieldEl) return false;
            if (el.x <= fieldEl.x) return false;
            const text = el.text.trim();
            return (
              text.includes(bFieldName) ||
    ...
    if (fieldNameElements.length === 1) { // This condition is redundant if we are already inside this block, but keeping for now
        ...
        if (fieldText.match(/[0-9][A|C]$/)) {
            ...
            const potentialPairElements = line.filter(el => {
                if (el === currentFieldEl) return false;
                if (el.x <= currentFieldEl.x) return false;
                const text = el.text.trim();
                if (expectedPairName.includes("GARAM MASALA 1D") || expectedPairName.includes("GARAM MASALA 2B")) {
                  debugLog(`[DEBUG ADJACENT PAIR CHECKING] Expected: "${expectedPairName}", Checking element text: "${text}"`);
                }
                return (
                    text.includes(expectedPairName) || 
...
export function initializeFieldBlocks(
  detectedFields: { leftFieldEl?: ExtractedTextElement, rightFieldEl?: ExtractedTextElement },
  headerLine: ExtractedTextElement[],
  midPointX: number,
  debugLog: DebugLogUtil
): { currentLeftBlock: FieldBlockInfo | null, currentRightBlock: FieldBlockInfo | null } {
...
export function extractGamesFromLineForBlock(
  lineElements: ExtractedTextElement[],
  blockInfo: FieldBlockInfo,
  midPointX: number, // To determine element boundaries for a block
  isLeftBlock: boolean,
  otherBlockInfo: FieldBlockInfo | null | undefined, // For GARAM MASALA 1A/1B reassignment logic
  debugLog: DebugLogUtil,
  pageWidth: number // For more context if needed, though primarily midPointX is used for boundaries
): GameInfo[] {
  const gamesOutput: GameInfo[] = [];
  if (!blockInfo) return gamesOutput;

  const blockStartX = blockInfo.startX;
  let relevantElements: ExtractedTextElement[];
  ...
  if (relevantElements.length > 0 && relevantElements[0].text.match(/^\d{2}\.\d{2}\s*-\s*\d{2}\.\d{2}$/)) {
    ...
    let team1 = "";
    let team2 = "";
    let team1Element: ExtractedTextElement | null = null;
    let team2Element: ExtractedTextElement | null = null;

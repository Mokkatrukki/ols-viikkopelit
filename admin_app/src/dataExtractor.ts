                ...
                if (fieldNumber.endsWith('A')) {
                    ...
                    if (existingBField) {
                        ...
                        const potentialBFields = line.filter(el => {
                            if (el === fieldEl) return false;
                            if (el.x <= fieldEl.x) return false; // Must be to the right
        ...
        if (currentRightBlock) {
            ...
            if (currentLeftBlock.name === "GARAM MASALA 1A" && 
                ...
                const existingGame = games.find(g => 
                    g.field === "GARAM MASALA 1B" && g.time === time
                );
                ...
                if (!existingGame) {
                    ...
                    const potentialRightTeamElements = line.filter(el => 
                        el.x > midPointBetweenFields && 
                        !el.text.match(/^\d{2}\.\d{2}\s*-\s*\d{2}\.\d{2}$/)
                    ).sort((a, b) => a.x - b.x);

...
pdfParser.on('pdfParser_dataReady', async (pdfData) => {
    ...
    try {
        ...
        if (pdfData.Pages && Array.isArray(pdfData.Pages)) {
            ...
            pdfData.Pages.forEach((page, pageIndex) => {
                if (page.Texts && Array.isArray(page.Texts)) {
                    ...
                    const masalaElements = page.Texts.filter((text) => {
                        if (text.R && Array.isArray(text.R)) {
                            return text.R.some((run) => {
                                if (run.T) {
                                    try {
                    ...
                    if (masalaElements.length > 0) {
                        ...
                        masalaElements.forEach((masalaElement) => {
                            ...
                            const nearbyElements = page.Texts.filter((text) => {
                            ...
                            if (nearbyElements.length > 0) {
                                debugLog(`Found ${nearbyElements.length} elements near GARAM MASALA 1B at y=${masalaY}`, nearbyElements.map((el) => ({
                                    x: el.x,
                                    y: el.y,
        ...
        fs.writeFile(outputJsonFilePath, jsonString, (err) => {
            if (err) {
